-- Function to create booking with PayPal authorization (not captured yet)
CREATE OR REPLACE FUNCTION public.create_booking_with_paypal_authorization(
    p_car_id uuid,
    p_start_date date,
    p_end_date date,
    ...
    p_paypal_authorization_id text,
    p_amount_authorized numeric
)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    -- 3. Create the booking record with authorized status
    ...
    VALUES (
        ...
        p_end_date,
        p_total_price,
        'authorized'::public.payment_status_enum,
    ...
    -- 4. Create the payment record with authorization details
    ...
    VALUES (
        ...
        'USD', -- Assuming USD
        p_paypal_order_id,
        p_paypal_authorization_id,
        NULL -- Not captured yet
    )
    RETURNING id INTO v_payment_id;

    -- 5. Temporarily reserve car availability (not fully booked until payment is captured)
    UPDATE public.car_availability
    SET 
        status = 'pending_confirmation'::public.car_availability_status_enum,
    ...
    VALUES (
        v_booking_id,
        'payment_authorized'::public.booking_event_type_enum,
        'webhook_paypal'::public.actor_type_enum,
        'Payment successfully authorized via PayPal.',
        jsonb_build_object(
            'paypalOrderId', p_paypal_order_id,
            'paypalAuthorizationId', p_paypal_authorization_id,
            'amount', p_amount_authorized
        )
    );
