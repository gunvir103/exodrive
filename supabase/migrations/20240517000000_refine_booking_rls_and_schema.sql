...
ALTER TABLE public.bookings ENABLE ROW LEVEL SECURITY; -- Already enabled, but good to ensure
...
CREATE POLICY "Admins can manage all bookings" 
  ...
  USING ((auth.jwt() ->> 'role') = 'admin')
  WITH CHECK ((auth.jwt() ->> 'role') = 'admin'); -- WITH CHECK for insert/update by admin


-- 5) RLS for payments table
ALTER TABLE public.payments ENABLE ROW LEVEL SECURITY; -- Already enabled
