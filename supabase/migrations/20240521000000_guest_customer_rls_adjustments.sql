...
DROP CONSTRAINT IF EXISTS customers_user_id_key; -- Removes the UNIQUE constraint if it was named this (common default)
...
CREATE POLICY "Authenticated users can insert bookings"
  ON public.bookings FOR INSERT
  TO authenticated
  WITH CHECK (true); -- Further checks on customer_id linkage can be done in API or via `EXISTS` if needed
