...
export async function POST(
  ...
  try {
    ...
    // Create and send rental agreement document via DocuSeal
    try {
      console.log(`Creating rental agreement for booking ${bookingId}`);
      const docusealResult = await docuSealService.createRentalAgreement(bookingId);
      
      console.log(`Rental agreement created successfully for booking ${bookingId}`, {
        submissionId: docusealResult.id,
      ...
      await supabase
        ...
        .insert({
          ...
          actor_type: 'system',
          summary_text: 'Rental agreement document created and sent to customer',
          details: {
            docuseal_submission_id: docusealResult.id,
            sent_to: docusealResult.submitters?.[0]?.email,
          },
        });
    } catch (docusealError) {
      // Log the error but don't fail the payment capture
      console.error(`Failed to create rental agreement for booking ${bookingId}:`, docusealError);
      ...
      await supabase
        ...
        .insert({
          ...
          actor_type: 'system',
          summary_text: 'Failed to create rental agreement document',
          details: {
            error: docusealError instanceof Error ? docusealError.message : 'Unknown error',
          },
        });
    }
