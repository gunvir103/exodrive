...
export default function WebhookMonitoringPage() {
  ...
  const processRetries = async () => {
    try {
      ...
      if (!response.ok) throw new Error('Failed to process retries');
      ...
      toast.error('Failed to process webhook retries');
    } finally {
      setProcessing(false);
    }
  };
  ...
  const retryDeadLetterItem = async (itemId: string) => {
    try {
      const { error } = await supabase.rpc('calculate_webhook_retry_time', {
        attempt_count: 0
      }).then(({ data: nextRetryTime }) => 
        supabase
